...
public class VkVideoPlaybackTestNegative {
    ...
    private static final By VIDEO_PLAY_BUTTON = By.id("com.vk.vkvideo:id/video_play_button");
    ...
    @Test
    @DisplayName("VK Видео: после паузы время не увеличивается")
    void afterPauseTimeDoesNotIncrease() {
        ...
        else $(VIDEO_DISPLAY).shouldBe(visible);

        tapToShowPlayerControls();
        sleep(500);
        clickPauseButton();
        int t1 = getCurrentTimeSeconds();
        assertTrue(t1 >= 0, "Не удалось прочитать текущее время");
        sleep(4000);
        int t2 = getCurrentTimeSeconds();
        assertTrue(t2 == t1, "После паузы время не должно расти: было " + t1 + ", стало " + t2);
    }

    private void clickPauseButton() {
        By pauseBy = null;
        if ($(VIDEO_PLAY_BUTTON).exists()) {
            String state = $(VIDEO_PLAY_BUTTON).getAttribute("content-desc");
            if (state != null && (state.equalsIgnoreCase("Pause") || state.contains("ауза"))) pauseBy = VIDEO_PLAY_BUTTON;
        }
        if (pauseBy == null && $(PLAYER_PLAY_PAUSE_BUTTON).exists()) {
            String state = $(PLAYER_PLAY_PAUSE_BUTTON).getAttribute("content-desc");
            if (state != null && (state.equalsIgnoreCase("Pause") || state.contains("ауза"))) pauseBy = PLAYER_PLAY_PAUSE_BUTTON;
        }
        if (pauseBy == null) {
            By x = By.xpath("//*[@content-desc='Pause' or @content-desc='Пауза' or contains(@content-desc,'ause')]");
            if ($(x).exists()) pauseBy = x;
        }
        assertTrue(pauseBy != null, "Кнопка Пауза не найдена");
        $(pauseBy).shouldBe(visible, Duration.ofSeconds(5));
        for (int attempt = 0; attempt < 2; attempt++) {
            try {
                WebElement el = $(pauseBy).getWrappedElement();
